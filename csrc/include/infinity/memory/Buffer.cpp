...
Buffer::Buffer(infinity::core::Context* context, uint64_t sizeInBytes) {
  ...
  INFINITY_ASSERT(this->ibvMemoryRegion != NULL,
...
Buffer::Buffer(infinity::core::Context* context,
...
Buffer::Buffer(infinity::core::Context* context,
  ...
  if (uint64_t(this->data) % infinity::core::Configuration::PAGE_SIZE) {
    uint64_t head =
        infinity::core::Configuration::PAGE_SIZE -
        uint64_t(this->data) % infinity::core::Configuration::PAGE_SIZE;
    temp += head;
  }
  cudaMemset(this->data, 0, cap);

  this->ibvMemoryRegion =
      ibv_reg_mr(this->context->getProtectionDomain(), temp, this->sizeInBytes,
                 IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE |
                     IBV_ACCESS_REMOTE_READ);
  INFINITY_ASSERT(this->ibvMemoryRegion != NULL,
                  "[INFINITY][MEMORY][BUFFER] Registration failed.\n");
...
void Buffer::resize(uint64_t newSize, void* newData) {
  ...
  if (memoryRegistered) {
    ...
    INFINITY_ASSERT(false,
                    "[INFINITY][MEMORY][BUFFER] You can only resize memory "
                    "which has registered by this buffer.\n");
  }
}
