...
void Context::postReceiveBuffer(infinity::memory::Buffer* buffer) {
  INFINITY_ASSERT(
      buffer->getSizeInBytes() <= std::numeric_limits<uint32_t>::max(),
      "[INFINITY][CORE][CONTEXT] Cannot post receive buffer which is larger "
      "than max(uint32_t).\n");
...
bool Context::receive(infinity::memory::Buffer** buffer,
