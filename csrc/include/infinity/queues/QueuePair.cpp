...
QueuePair::QueuePair(infinity::core::Context* context) : context(context) {
  ...
  // All qps in the same context share the same send_cq and recv_cq
...
void QueuePair::write(infinity::memory::Buffer* buffer,
  ...
  INFINITY_ASSERT(
      sizeInBytes <= destination->getRemainingSizeInBytes(remoteOffset),
      "[INFINITY][QUEUES][QUEUEPAIR] Segmentation fault while writing to "
      "remote memory.\n");
...
void QueuePair::multiWrite(infinity::memory::Buffer** buffers,
  ...
  INFINITY_ASSERT(
      totalSizeInBytes <= destination->getRemainingSizeInBytes(remoteOffset),
      "[INFINITY][QUEUES][QUEUEPAIR] Segmentation fault while writing to "
      "remote memory.\n");
...
void QueuePair::multiRead(uint32_t batch_size,
                          infinity::memory::Buffer* buffer,
                          int64_t* localOffset,
                          infinity::memory::RegionToken* source,
  ...
  INFINITY_ASSERT(
      sizeInBytes <= buffer->getRemainingSizeInBytes(localOffset[0]),
      "[INFINITY][QUEUES][QUEUEPAIR] Segmentation fault while "
      "creating scatter-getter element.\n");

  INFINITY_ASSERT(
      sizeInBytes <= source->getRemainingSizeInBytes(remoteOffset[0]),
      "[INFINITY][QUEUES][QUEUEPAIR] Segmentation fault while "
      "reading from remote memory.\n");
