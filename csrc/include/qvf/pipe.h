...
namespace qvf{
    ...
    struct PipeParam{
        ...
        void set_params(int qp_num, int cq_mode, int ctx_poll_batch, int tx_depth, int post_list_size){
            this->qp_num = qp_num;
            this->cq_mode = cq_mode;
            this->ctx_poll_batch = ctx_poll_batch;
            this->tx_depth = tx_depth;
        ...
        public:
            ...
            void connect(){
                ...
                for(int qp_index = 0; qp_index < pipe_param.qp_num; qp_index++){
                    qps[qp_index] = qpFactory->connectToRemoteHost(remote_end.get_address().c_str(), remote_end.get_port());
                }
