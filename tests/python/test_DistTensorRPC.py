...
shard_tensor_config = ShardTensorConfig({args.local_rank: "8G"})
...
if args.cpu_collect or args.cpu_collect_gpu_send:
    ...
    dist_tensor = DistTensorRPC(args.world_size, args.rank, args.device_per_node, args.local_rank, tensor, range_list, rpc_option, cached_range, **debug_param)
else:
    dist_tensor = DistTensorRPC(args.world_size, args.rank, args.device_per_node, args.local_rank, shard_tensor, range_list, rpc_option, cached_range, **debug_param)

warm_up = 4
for idx in range(warm_up):
    data = dist_tensor[indices]
